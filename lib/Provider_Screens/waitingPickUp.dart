...
class waitingForPick extends StatefulWidget {
  Order_object order;
  waitingForPick({super.key, required this.order});
  @override
  _waitingForPickState createState() => _waitingForPickState();
}

class _waitingForPickState extends State<waitingForPick> {
  Timer? timer;
  ...
  changeOrderStatus() {
    ...
    db.updateOrderInfo(widget.order.getorderID, {
      'status': OrderStatus.pickedUp.toString().replaceAll('OrderStatus.', '')
    });
  }
